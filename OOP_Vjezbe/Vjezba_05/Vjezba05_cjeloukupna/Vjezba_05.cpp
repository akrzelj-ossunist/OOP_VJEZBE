	...
	for (G.Ammo; G.Ammo > G.Clip_ammo; G.Ammo -= G.Clip_ammo, cnt++)
	{
		for (int i = G.Clip_ammo; i != 0; i--)
		{
			...
			for (int j = 0; j < num; j++)
			{
				...
				if (T[j].Dot_1.z >= T[j].Dot_2.z)
				{
					if (G.Position.z <= T[j].Dot_1.z && G.Position.z >= T[j].Dot_2.z)
						br++;
				}
				else
				{
					if (G.Position.z <= T[j].Dot_2.z && G.Position.z >= T[j].Dot_1.z)
						br++;
				}
				if (br == 2)
				{
...
void main()
{
	...
	/*for (int i = 0; i < num; i++)
	{
		cout << T[i].Dot_1.x << "  " << T[i].Dot_1.y << T[i].Dot1.z < endl;
		cout << T[i].Dot_2.x << "  " << T[i].Dot_2.y << T[i].Dot2.z << endl;
		puts(" ");
	}*/
